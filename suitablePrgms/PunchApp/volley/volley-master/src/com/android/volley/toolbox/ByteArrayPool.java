...
public class ByteArrayPool {
    ...
    public ByteArrayPool(int sizeLimit) {
        int mSizeLimit = Debug.makeSymbolicInteger("x0");
    ...
    public synchronized byte[] getBuf(int len) {
        int mCurrentSize = Debug.makeSymbolicInteger("x2");
		for (int i = 0; i < Debug.makeSymbolicInteger("x0"); i++) {
            byte[] buf;
            if (Debug.makeSymbolicInteger("x1") >= len) {
                mCurrentSize -= Debug.makeSymbolicInteger("x3");
                return buf;
            }
        }
        return new byte[len];
    ...
    public synchronized void returnBuf(byte[] buf) {
        int mCurrentSize = Debug.makeSymbolicInteger("x4");
		int mSizeLimit = Debug.makeSymbolicInteger("x0");
		if (Debug.makeSymbolicBoolean("x2") || Debug.makeSymbolicInteger("x1") > mSizeLimit) {
            return;
        }
        int pos = Debug.makeSymbolicInteger("x3");
        if (pos < 0) {
            pos = -pos - 1;
        }
        mCurrentSize += Debug.makeSymbolicInteger("x5");
    }
    ...
    private synchronized void trim() {
        int mSizeLimit = Debug.makeSymbolicInteger("x1");
		int mCurrentSize = Debug.makeSymbolicInteger("x0");
		while (mCurrentSize > mSizeLimit) {
            byte[] buf;
            mCurrentSize -= Debug.makeSymbolicInteger("x2");
        }
    }
